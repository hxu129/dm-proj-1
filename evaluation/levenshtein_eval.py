...
def main(parquet_path: str,
    ...
    for pair in tqdm(cand, disable=len(cand) < 100, desc='levenshtein'):
        t1 = txt.get(pair['wikidata_id'], '')
        for wid in pair.get('similar_items', []):
            sim = _ratio(t1, txt.get(wid, ''))
            if sim < threshold:
                continue
            sims.append(sim)
            ...
            res.append({'wikidata_id': pair['wikidata_id'],
                        'similar_to': wid,
                        'levenshtein_ratio': sim})

    total = len(sims)
    summary = {
        'total_pairs_evaluated': total,
        'average_levenshtein_ratio': sum(sims) / total if total else 0.0,
    ...
    print(f'done: {total} pairs, '
          f'avg {summary["average_levenshtein_ratio"]:.4f}, '
